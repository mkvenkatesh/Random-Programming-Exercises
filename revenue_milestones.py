...
Output
Return a length-K array where K_i is the day on which FB first had milestones[i]
total revenue. If the milestone is never met, return -1.
...
def getMilestoneDays(revenues, milestones):
    ...
    # add index of the array to milestones to keep track of indices and sort the
    # array on milestones

    # loop over revenues with a running sum and check to see if first milestone
    # is hit, if so loop over milestones until all the others are marked which
    # are <= running sum and repeat

    output = [-1] * len(milestones)

    for idx, val in enumerate(milestones):
        milestones[idx] = [val, idx]
  
    milestones.sort(key=lambda x:x[0])

    running_sum = 0
    mile_idx = 0
    current_milestone = milestones[mile_idx][0]
    current_milestone_idx = milestones[mile_idx][1]
    for idx, val in enumerate(revenues):
        running_sum += val
        while (running_sum >= current_milestone) and (mile_idx < len(milestones)):
            output[current_milestone_idx] = idx + 1
            mile_idx += 1
